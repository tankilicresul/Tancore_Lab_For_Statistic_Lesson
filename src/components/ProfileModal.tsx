...
import { uploadAvatarImage, deleteUserAvatar, saveUserProfileToSupabase, updateUserAccountCredentials } from '../lib/supabase';
...
export const ProfileModal: React.FC<ProfileModalProps> = ({ onClose, onOpenAuth }) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSaveStatus(language === 'tr' ? 'Hesap bilgileriniz güncellendi!' : 'Account updated successfully!');
      setTimeout(() => {
        setIsEditing(false);
        setEditPassword('');
        setSaveStatus(null);
          ...
          <div className="bg-slate-900 rounded-2xl p-4 sm:p-5 text-white shadow-lg relative overflow-hidden">
            ...
            {isEditing ? (
              <form onSubmit={handleSave} className="mt-4 pt-3 border-t border-white/10 space-y-2.5 relative z-10">
                {saveStatus && (
                  <div className={`p-2 rounded-xl text-[11px] font-bold flex items-center space-x-2 ${saveStatus.includes('başarıyla') || saveStatus.includes('successfully') ? 'bg-emerald-500/20 border border-emerald-500/40 text-emerald-200' : 'bg-rose-500/20 border border-rose-500/40 text-rose-200'}`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-slate-300 block mb-1">
                      ...
                      <span className="text-[9px] text-slate-400 font-normal ml-1">({language === 'tr' ? 'İsteğe bağlı' : 'Optional'})</span>
                    </label>
                    <input
                      type="text"
                  ...
                  <div className="sm:col-span-2">
                    <label className="text-[10px] font-bold text-slate-300 block mb-1">
                      ...
                      <span className="text-[9px] text-slate-400 font-normal ml-1">({language === 'tr' ? 'Değiştirmek istemiyorsanız boş bırakın' : 'Leave blank to keep'})</span>
                    </label>
                    <div className="relative">
                      <Lock className="w-3.5 h-3.5 text-white/40 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" />
                      <input
                        type={showEditPassword ? 'text' : 'password'}
                        placeholder={language === 'tr' ? 'Yeni şifreniz (en az 4 karakter)' : 'New password (min 4 chars)'}
                        value={editPassword}
                        onChange={(e) => setEditPassword(e.target.value)}
                        className="w-full pl-8 pr-9 py-1.5 rounded-xl bg-white/10 border border-white/20 text-white text-xs font-medium focus:outline-none focus:border-[#ff7a00]"
                      />
                      <button
                        type="button"
                ...
                <div className="pt-1 flex items-center justify-between">
                  <button
                    ...
                    disabled={isSaving}
                    className="flex items-center space-x-1 px-3 py-1.5 rounded-xl bg-white/10 hover:bg-white/20 text-white text-xs font-bold transition-colors border border-white/15 cursor-pointer disabled:opacity-50"
                  >
                  ...
                  <button
                    type="submit"
                    disabled={isSaving}
                    className="flex items-center space-x-1.5 px-3.5 py-1.5 rounded-xl bg-[#ff7a00] hover:bg-[#e66e00] text-white text-xs font-bold transition-colors shadow-sm cursor-pointer disabled:opacity-50 active:scale-95"
                  >
                    {isSaving ? (
                      <>
