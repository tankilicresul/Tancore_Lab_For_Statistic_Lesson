      ...
      <div className="space-y-3.5">
        ...
        {data.excelFormula && (
          <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800">
            <div className="flex items-center justify-between mb-2">
              <span className="flex items-center space-x-2 text-xs font-bold text-amber-400">
                <FileSpreadsheet className="w-4 h-4" />
                <span>Excel Formülü</span>
              </span>
              <button
                onClick={() => handleCopy(data.excelFormula!, 'excel')}
                className="flex items-center space-x-1.5 text-[11px] font-semibold text-slate-400 hover:text-white transition-colors"
              >
                {copiedKey === 'excel' ? <Check className="w-3.5 h-3.5 text-amber-400" /> : <Copy className="w-3.5 h-3.5" />}
                <span>{copiedKey === 'excel' ? 'Kopyalandı' : 'Kopyala'}</span>
              </button>
            </div>
            <pre className="font-mono text-xs text-amber-300 bg-slate-900 p-3 rounded-xl overflow-x-auto border border-slate-800 font-medium">
              {data.excelFormula}
            </pre>
          </div>
        )}
          ...
          <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800">
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                className="flex items-center space-x-1.5 text-[11px] font-semibold text-slate-400 hover:text-white transition-colors"
              >
                {copiedKey === 'python' ? <Check className="w-3.5 h-3.5 text-amber-400" /> : <Copy className="w-3.5 h-3.5" />}
                <span>{copiedKey === 'python' ? 'Kopyalandı' : 'Kopyala'}</span>
              </button>
            </div>
            <pre className="font-mono text-xs text-amber-200 bg-slate-900 p-3 rounded-xl overflow-x-auto border border-slate-800 font-medium">
              {data.pythonCode}
            </pre>
          </div>
        )}
          ...
          <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800">
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                className="flex items-center space-x-1.5 text-[11px] font-semibold text-slate-400 hover:text-white transition-colors"
              >
                {copiedKey === 'sql' ? <Check className="w-3.5 h-3.5 text-amber-400" /> : <Copy className="w-3.5 h-3.5" />}
        ...
        {data.powerBiNote && (
          <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800">
            ...
            <p className="text-xs text-slate-200 bg-slate-900 p-3 rounded-xl border border-slate-800 leading-relaxed font-medium">
